...
const ProjectDetail = ({ project, onBack }) => (
  <div className="py-12 animate-fade-in max-w-4xl mx-auto">
    <button 
      onClick={onBack} 
      className="cursor-pointer group mb-8 inline-flex items-center gap-2 text-sm font-medium text-gray-400 transition-colors hover:text-primary-400"
    >
        ...
        <div className="p-6 sm:p-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                ...
                <div className="flex gap-4">
                    <button onClick={()=> window.open(project.repoUrl,'_blank')}  className="cursor-pointer flex items-center gap-2 text-sm font-medium text-gray-300 hover:text-primary-400 transition-colors border border-gray-700 rounded-full px-4 py-2 hover:bg-gray-800">
                        <Github size={18}/> Repository
                    </button>
                    <button onClick={()=> window.open(project.liveUrl,'_blank')} className="cursor-pointer group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-white px-6 py-2.5 text-sm font-semibold text-black transition-transform hover:scale-100 hover:bg-gray-200">
                            <span>Live Demo</span>
